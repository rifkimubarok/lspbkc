...
<div class="single-post-wrap">
	<div class="content">
		<h3 class="pb-30">Registrasi Calon Pasukan Utama</h3>
		<form id="frm_registrasi">
			<div class="row">
					...
					<div class="form-group col-md-6">
						<label for="email">Alamat Email<span style="color: red" title="Wajib diisi">*</span> </label>
					...
					<div class="form-group col-md-4">
            <label for="provinsi">Provinsi<span style="color: red" title="Wajib diisi">*</span></label>
            <select name="provinsi" id="provinsi" required="" class="form-control">
            </select>
          </div>

          <div class="form-group col-md-4">
            <label for="kabupaten">Kabupaten<span style="color: red" title="Wajib diisi">*</span></label>
            <select name="kabupaten" id="kabupaten" required="" class="form-control">
            </select>
          </div>

          <div class="form-group col-md-4">
            <label for="kecamatan">Kecamatan<span style="color: red" title="Wajib diisi">*</span></label>
            <select name="kecamatan" id="kecamatan" required="" class="form-control">
            </select>
          </div>

          <div class="form-group col-md-4">
						<label for="status_pendidikan">Status<span style="color: red" title="Wajib diisi">*</span></label>
						<select name="status_pendidikan" id="status_pendidikan" required="" class="form-control">
              <option value="1">Pelajar/SLTA</option>
              <option value="2">Mahasiswa</option>
						</select>
					</div>

          <div class="form-group col-md-8">
            <label for="alamat_instansi">Alamat Institusi Pendidikan<span style="color: red" title="Wajib diisi">*</span></label>
            <textarea name="alamat_instansi" id="alamat_instansi" required="" class="form-control"></textarea>
          </div>
					<!-- <div class="form-group col-md-4">
            ...
            <input type="text" name="tahun_krn" id="tahun_krn" required="" class="form-control">
					...
					<div class="form-group col-md-6">
						<label for="ktp_file">Scan Identitas(Kartu Pelajar/Mahasiswa)</label><br>
...
<script type="text/javascript">
	...
	function get_ref_status() {
   	...
   	$('#frm_registrasi').submit(function() {
   		$.ajax({
   			url:getUri(app_registrasi,"register_calon"),
   			type:"post",
   			dataType:"JSON",
    ...
    function get_provinsi(combobox = null,kode_prov = null){
        $.ajax({
            url:getUri(app_registrasi,'get_provinsi'),
            type:"post",
            dataType:"json",
            success:function(result){
                var option = '<option value="" selected>Pilih Provinsi</option>';
                for(var i = 0; i< result.provinsi.length;i++){
                    var data = result.provinsi[i]['name'];
                    var kode = result.provinsi[i]['kode'];
                    option += '<option value="'+kode+'">'+data+'</option>';
                }
                 option += '<option value="0" selected>Luar Negeri</option>';
                combobox.html(option);
                combobox.val(kode_prov);
                return true;
            },
    ...
    function get_kabupaten(combobox = null,combobox2= null,kode_prov=null,kode_kab=null){
        $.ajax({
            url:getUri(app_registrasi,'get_kabupaten'),
            type:"post",
            dataType:"json",
            data:{kode_prov:kode_prov},
            beforeSend:function(){
                var option =  '<option value="" selected>Pilih Kabupaten/Kota</option>';
                combobox.html(option);
                var option = '<option value="" selected>Pilih Kecamatan</option>';
                combobox2.html(option);
            },
            success:function(result){
                var option = '<option value="" selected>Pilih Kabupaten/Kota</option>';
                if(result.kabupaten != null){
                    for(var i = 0; i< result.kabupaten.length;i++){
                        var data = result.kabupaten[i]['nama'];
                        var kode = result.kabupaten[i]['kode'];
                        option += '<option value="'+kode+'">'+data+'</option>';
                    }
                     option += '<option value="0">Kabupaten Lainnya</option>';
                }
                combobox.html(option);
                if(kode_kab != null){
                    combobox.val(kode_kab);
                }
            },
            error:function(x,y,z){
                alert('Terjadi Sebuah Kesalahan.');
    ...
    function get_kecamatan(combobox = null,combobox2 = null,kode_kab=null,kode_kec=null){
        $.ajax({
            url:getUri(app_registrasi,'get_kecamatan'),
            type:"post",
            dataType:"json",
            ...
            beforeSend:function(){
                var option = '<option value="" selected>Pilih Kecamatan</option>';
                combobox.html(option);
                var option = '<option value="" selected>Pilih Kelurahan/Desa</option>';
                combobox2.html(option);
            },
            success:function(result){
                var option = '<option value="" selected>Pilih Kecamatan</option>';
                if (result.kecamatan != null) {
                    for(var i = 0; i< result.kecamatan.length;i++){
                        var data = result.kecamatan[i]['nama'];
                        var kode = result.kecamatan[i]['kode'];
                        option += '<option value="'+kode+'">'+data+'</option>';
                    }
                     option += '<option value="0">Kecamatan Lainnya</option>';
                }
                combobox.html(option);
                if(kode_kec != null){
                    combobox.val(kode_kec);
                }
            },
            error:function(x,y,z){
                alert('Terjadi Sebuah Kesalahan.');
    ...
    function get_kelurahan(combobox = null,kode_kec=null,kode_kel=null){
        $.ajax({
            url:getUri(app_registrasi,'get_kelurahan'),
            type:"post",
            dataType:"json",
            ...
            success:function(result){
                ...
                if (result.kelurahan != null) {
                    for(var i = 0; i< result.kelurahan.length;i++){
                        var data = result.kelurahan[i]['nama'];
                        var kode = result.kelurahan[i]['kode'];
                        option += '<option value="'+kode+'">'+data+'</option>';
                    }
                 option += '<option value="0" selected>Kelurahan/Desa Lainnya</option>';
                }
                combobox.html(option);
                if(kode_kel != null){
                    combobox.val(kode_kel);
                }
            },
            error:function(x,y,z){
                alert('Terjadi Sebuah Kesalahan.');
...
$('#provinsi').on('change',function(){
    if($(this).val() == null || $(this).val() == ''){
        ...
        get_kabupaten($('#kabupaten'),$('#kecamatan'),$('#provinsi').val());
    }
...
$('#kabupaten').on('change',function(){
    if($(this).val() == null || $(this).val() == ''){
        ...
        get_kecamatan($('#kecamatan'),$('#kelurahan'),$('#kabupaten').val());
    }
...
$('#kecamatan').on('change',function(){
    if($(this).val() == null || $(this).val() == ''){
        ...
        get_kelurahan($('#kelurahan'),$('#kecamatan').val());
    }
});
