					...
					foreach ($komentar->data as $item) {
						...
						<div class="single-comment justify-content-between d-flex">
							<div class="user justify-content-between d-flex">
								<div class="thumb profile-picture">
									<img src="<?=BASEURL?>api/pictures/show_picture/member_docs/thumb_foto_<?=$item['komentar']->member_id?>" alt="" style="width: 100%;">
								</div>
								<div class="desc">
									<h5><a href="#"><?=$item['komentar']->nama_pengirim?></a></h5>
									<p class="date"><?=date("d F,Y H:i",strtotime($item['komentar']->date_insert))?> </p>
									<p class="comment">
							...
							foreach ($item['reply_komentar']->data as $reply) {
								?>
										<div class="single-comment left-padding justify-content-between d-flex">
											<div class="user justify-content-between d-flex">
												<div class="thumb profile-picture">
													<img src="<?=BASEURL?>api/pictures/show_picture/member_docs/thumb_foto_<?=$reply->member_id?>" alt="" style="width: 100%;">
												</div>
												<div class="desc">
													<h5><a href="#"><?=$reply->nama_pengirim?></a></h5>
													<p class="date"><?=date("d F,Y H:i",strtotime($reply->date_insert))?> </p>
													<p class="comment">
														<?=$reply->komentar?>
													</p>
												</div>
...
<?php if ($islogin) {
	...
	<div style="display: none;">
		<div class="single-comment left-padding justify-content-between d-flex kolom_komentar" id="">
			<div class="user justify-content-between d-flex">
				<div class="thumb profile-picture">
					<img src="<?=BASEURL?>api/pictures/show_picture/member_docs/thumb_foto_<?=$user_id?>" alt="" style="width: 100%;">
				</div>
				<div class="desc">
...
<script type="text/javascript">
	...
	function reply_comment(item) {
		...
		var btn = $(item);
		$.ajax({
			url:getUri(app_forum,'reply_comment'),
			type:"post",
